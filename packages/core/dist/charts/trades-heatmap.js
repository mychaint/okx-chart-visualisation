...
export async function renderTradesHeatmap(req) {
    const W = 800, H = 350;
    const trades = (req.data?.data ?? []);
    if (trades.length === 0)
        throw new Error('No trades data');
    ...
    const grid = Array.from({ length: PRICE_BINS }, () => Array(TIME_BINS).fill(0));
    trades.forEach(t => {
    ...
    grid.forEach((row, pIdx) => {
        row.forEach((val, tIdx) => {
            ...
            ctx.fillRect(pane.x + tIdx * cellW, pane.y + (PRICE_BINS - 1 - pIdx) * cellH, cellW, cellH);
        });
    });
...
//# sourceMappingURL=trades-heatmap.js.map