...
export async function renderPnl(req) {
    const W = 800, H = 300;
    const positions = (req.data?.data ?? [])
        .map((p) => ({ instId: p.instId, upl: Number(p.upl), uplRatio: Number(p.uplRatio) }));
    ...
    ctx.lineTo(centerX, paddingTop + chartH);
    ctx.stroke();
    ctx.setLineDash([]);
